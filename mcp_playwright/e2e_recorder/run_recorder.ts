      ...
      try {
        ...
        if (action.type === 'navigation') {
          ...
          const locator = action.role
            ? page.getByRole(action.role as any, { name: action.selector, exact: true }).first()
...
function saveRecording() {
  ...
  fs.writeFileSync(RECORDING_PATH, JSON.stringify({ meta: { timestamp: Date.now() }, actions: events }, null, 2));
}
